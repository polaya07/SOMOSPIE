...
def pipeline(
    container_image: str ="icr.io/somospie/somospie-gdal",
    total_tiles: int = 3,
    eval_cos: str = "oklahoma-30m",
    train_cos: str = "oklahoma-27km",
    predict_cos: str = "p-oklahoma-30m",
    mem_lim: int = 13,
    random_seed: int = 3,
    k_neighbors: int = 20,
    trees: int = 2000
    ...
    data_task = data_op("/cos/train.csv", "/cos/", "/cos/data.json").add_pvolumes({"/cos/": pvc_train.volume}).set_retry(3)
    # Train after splitting the data
    ## KNN:
    #knn_train_task = knn_train_op(data_task.outputs['data'], k_neighbors, random_seed, "/cos/model_knn.pkl").add_pvolumes({"/cos/": pvc_train.volume}).set_retry(3)
    rf_train_task = rf_train_op(data_task.outputs['data'], trees, random_seed, "/cos/model_rf.pkl").add_pvolumes({"/cos/": pvc_train.volume}).set_retry(3)

    total_tiles = 9 #Here is the total tiles
    ...
    for i in range(total_tiles):
        ...
        tile_id = "eval_tile_{0:04d}".format(i)
        ## Read Data:
        #tif2df_task.append(tif2df_op("/eval/"+tile_id+".csv", band_names))
        ## KNN:
        #knn_inference_task = knn_inference_op(knn_train_task.output, data_task.outputs['scaler'], "/eval/"+tile_id+".tif" , "/out_knn/knn/",
        #                    "/out_knn/knn/"+tile_id+".csv", band_names).add_pvolumes({"/cos/": pvc_train.volume, "/eval/": pvc_eval.volume, 
        ...
        ## RF:
        rf_inference_task = rf_inference_op(rf_train_task.output, data_task.outputs['scaler'],  "/eval/"+tile_id+".tif" , "/out_rf/rf/",
                            "/out_rf/rf/"+tile_id+".csv", band_names).add_pvolumes({"/cos/": pvc_train.volume, "/eval/": pvc_eval.volume, 
